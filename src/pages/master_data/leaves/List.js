...
const List = () => {
    ...
    let defaultFilters = {
        fields: ["dtLeaveType_id", "dtEmployee_id", "fromDate", "toDate", "status"],
        first: 0,
        rows: 10,
        page: 1,
        sortField: null,
        ...
        filters: {
            ...
            'fromDate': { operator: FilterOperator.OR, constraints: [{ value: null, matchMode: FilterMatchMode.STARTS_WITH }] },
            'toDate': { operator: FilterOperator.OR, constraints: [{ value: null, matchMode: FilterMatchMode.STARTS_WITH }] },
    ...
    const deleteProfile = () => {
        hrManagementService.delete(modelName, dtProfile._id).then(data => {
            console.log(data);
            loadLazyData();
            toast.current.show({ severity: 'success', summary: 'Successful', detail: 'Leave Profile Deleted', life: 3000 });
        });
        setDeleteProfileDialog(false);
        setProfile(null);
    };
